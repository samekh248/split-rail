...
- EF migration from this feature applied
...
2. Mock QBO fetch with empty/minus TXN-1.
3. Sync.
...
Integration test `QboAppendOnlyTests` (updated):
...
3. Assert `qboActualValue` updated; `settlementValue` unchanged; mutation auditor not triggered for sync path.
