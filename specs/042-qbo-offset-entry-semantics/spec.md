...
**Feature Branch**: `042-qbo-offset-entry-semantics`
...
**Depends on**: QBO pull cache and append-only sync ledger (SPLR-18), financial ledger grid and QBO actuals column (SPLR-17), platform constitution append-only QBO actuals mandate (Constitution IV)
...
- Amount changes, voids, and removals are in scope for automatic offset generation; manual user-initiated actuals edits remain out of scope (actuals stay read-only per existing ledger grid behavior).
...
- QBO actuals corrections are an explicit exception to settlement freeze for the read-only actuals aggregate only; all other financial line item fields on settled/reconciled events remain immutable.
- Offset amount calculation uses net-to-target semantics: offset amount equals current upstream amount minus the net sum of all ledger entries for that transaction identifier.
- A prior specification draft exists at `specs/040-qbo-offset-entry-semantics`; this feature directory supersedes it for the SPLR-37 implementation track on branch `042-qbo-offset-entry-semantics`.
